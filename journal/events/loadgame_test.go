...
func (s *EventSuite) TestHandleEventLoadGame(c *check.C) {
	...
	fuelCapacity := []float64{32.000000, 1.000000, 1.000000, 32.000000}
	fuelLevel := []float64{23.991510, 0.987092, 0.987092, 32.000000}
	...
	for i, rawEvent := range rawEvents {
		...
		c.Assert(status.Credits, check.Equals, credits[i])
		c.Assert(status.FuelCapacity, check.Equals, fuelCapacity[i])
		c.Assert(status.FuelLevel, check.Equals, fuelLevel[i])
	}
}
