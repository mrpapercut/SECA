// Thanks to https://github.com/mcmuttons/GeoPredictor/blob/master/RegionMap

const x0 = -49985;
// const y0 = -40985;
const z0 = -24105;

export default function getGalacticRegion(coordinates: Coordinates): string {
    const px = Math.floor((coordinates.x - x0) * 83 / 4096);
    const pz = Math.floor((coordinates.z - z0) * 83 / 4096);

    if (px < 0 || pz < 0 || pz >= regionMapLines.length) {
        return '';
    } else {
        const row = regionMapLines[pz];
        let rx = 0;
        let pv = 0;
